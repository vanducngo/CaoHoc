...
parser.add_argument(
    "--config",
    default="configs/args.yml",
    help="path of the config file (training only)",
)
parser.add_argument("--dataset", default="ImageNet", help="")
parser.add_argument(
    "--data_dir", default="", help="local path to data root dir from " + DATA_FOLDER
)
parser.add_argument(
    "--logpath", default=None, help="Path to dir where to logs are stored (test only)"
)
parser.add_argument(
    "--cv_dir", default="logs/", help="dir to save checkpoints and logs to"
)
...
parser.add_argument(
    "--test_batch_size", type=int, default=16, help="Batch size at test/eval time"
)
...
parser.add_argument(
    "--lrg", type=float, default=1e-3, help="Learning rate feature extractor"
)
parser.add_argument("--weight_decay", type=float, default=5e-5, help="Weight decay")
parser.add_argument(
    "--eval_val_every", type=int, default=1, help="Frequency of eval in epochs"
)
...
parser.add_argument(
    "--past_key_value_file", default="release_past_key_value.pt", type=str
)
...
parser.add_argument(
    "--clip_text_embed_file",
    default="release_clip_text_embeddings.pt",
    type=str,
    help="Đường dẫn đến tệp nhúng CLIP",
)
